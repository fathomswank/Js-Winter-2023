...
class Teacher {
    ...
    updateGradeOfStudent(studentId, studentUpdatedGrade) {
        const studentInfoObject = Student.findStudentInfoObjectById(studentId);
        if (!studentInfoObject) {
            console.log(`\nIncorrect student id "${studentId}" provided`);
       } else if (studentInfoObject.sGrade.localeCompare(studentUpdatedGrade) === 0) {
            console.log(`\nStudent with id "${studentId}" is already graded: ${sGrade}; to update student-grade pls use a different grade!`);
       } else {
            studentInfoObject.sGrade = studentUpdatedGrade;
            console.log(`\nThank you for updating the student's grade into a: ${studentUpdatedGrade}with id = ${studentId}`);
       } 
}
